...
describe("views.markSeen", () => {
  ...
  test("rejects a non-friend marking a friends-tier drop seen (COM-135)", async () => {
    const t = makeTest();
    const dropId = await seedDropFromAlice(t);
    await seedStranger(t, "carol");

    await expect(
      t.withIdentity({ subject: "user_carol" }).mutation(api.views.markSeen, { dropId }),
    ).rejects.toThrow(/forbidden/i);
    const drop = await t.run((ctx) => ctx.db.get(dropId));
    expect(drop?.viewCount).toBe(0);
  });

  test("rejects marking a private drop seen, even from a friend (COM-135)", async () => {
    const t = makeTest();
    ...
    await expect(
      t.withIdentity(asBob).mutation(api.views.markSeen, { dropId: privateDropId }),
    ).rejects.toThrow(/forbidden/i);
  });

  test("allows a non-friend to mark a public drop seen (COM-135)", async () => {
    const t = makeTest();
    ...
    await t
      ...
      .mutation(api.views.markSeen, { dropId: publicDropId });
    const drop = await t.run((ctx) => ctx.db.get(publicDropId));
    expect(drop?.viewCount).toBe(1);
  });
});
